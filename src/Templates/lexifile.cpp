...
void Lexi::BufferedFile::open(const char *name)
{
	char *			buffer;
	char *			newbuffer;
	bool			bHasCarriageReturns = false;
	...
	{
		if (c == '\r')
			bHasCarriageReturns = true;
		else if (c == '\n')
			++linefeeds;
		c = *p++;
	}
	
	if (!bHasCarriageReturns)
	{
		newbuffer = new char[length + linefeeds + 1];
...
void Lexi::OutputParserFile::WriteEnum(const char *tag, int val, const char **table)
{
	WriteTag(tag);
	*this << findtype(val, (const char **) table) << std::endl;
}
...
void Lexi::OutputParserFile::WriteFlagsAlways(const char *tag, int val, const char **table)
{
	BUFFER(buf, MAX_STRING_LENGTH);
	
	sprintbit(val, (const char **) table, buf);
...
void Lexi::OutputParserFile::WriteFlagsAlways(const char *tag, BitFlagsWrapper::Ptr wrapper, const char **table)
{
	WriteTag(tag);
