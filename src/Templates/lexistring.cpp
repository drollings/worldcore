...
#if defined(TRACK_STRING_BUFFERING)
String::String() : str(buf), buffered(true), len(0), cap(0) /* nulls the buf too */ 
{
...
String::String(const String &s) : str(buf), buffered(true), len(0), cap(0)	/* nulls the buf too */
{
#if defined(TRACK_STRING_BUFFERING)
	++g_numBufferedStrings;
	++g_numTotalStrings;
...
String::String(const String &s, size_type pos, size_type n) : str(buf), buffered(true), len(0), cap(0) 	/* nulls the buf too */
{
#if defined(TRACK_STRING_BUFFERING)
	++g_numBufferedStrings;
	++g_numTotalStrings;
...
String::String(const char *s) : str(buf), buffered(true), len(0), cap(0) /* nulls the buf too */
{
#if defined(TRACK_STRING_BUFFERING)
	++g_numBufferedStrings;
	++g_numTotalStrings;
...
String::String(const char *s, size_type n) : str(buf), buffered(true), len(0), cap(0) /* nulls the buf too */
{
#if defined(TRACK_STRING_BUFFERING)
	++g_numBufferedStrings;
	++g_numTotalStrings;
...
{
	bool isBuffered = is_buffered();
	if (isBuffered && s.is_buffered())
	{
		strcpy(buf, s.buf);
		len = s.len;
		return *this;
...
String::size_type String::find(const char * s, size_type pos, size_type n) const
{
	...
	if (pos < sz)
	{
		const char *p = str_str(beg + pos, s);
		if (p)
			return p - str;
	}
...
String::size_type String::find(char c, size_type pos) const
{
	...
	if (pos < sz)
	{
		for (const char *p = beg + pos, *e = beg + sz; p < e; ++p)
			if (tolower(*p) == c)
				return p - str;
	}
