...
void sub_write_to_char(Things::Thing *ch, char *tokens[], Ptr otokens[], char type[]);
...
void prompt_str(Things::Thing *ch, char *buf, int length);
void	perform_act(const char *orig, Things::Thing *ch, Things::Thing *obj,
		    const void *vict_obj, Things::Thing *to, int type, char *storeBuf);
...
void act_mtrigger(Things::Thing *ch, const char *str, Things::Thing *actor, Things::Thing *victim, Things::Thing *object, Things::Thing *target, const char *arg);
void act_wtrigger(Things::Thing *room, const char *str, Things::Thing *actor, Things::Thing *object);
...
void copyover_recover(void) {
	...
	for (;;) {
		...
		if (!d->character->Load(name) || (PLR_FLAGGED(d->character, PLR_DELETED) && !PLR_FLAGGED(d->character, PLR_NODELETE)))
		{
			fOld = FALSE;
		}
		else
		...
		{ /* ok! */
//			if (compressionMode)
//				compressStart(d, compressionMode);
			
			d->Write("\r\nCopyover recovery complete.\r\n");
			enter_player_game(d->character);
//			STATE(d) = CON_PLAYING;
...
#else
	sa.sin_addr.s_addr = inet_addr("204.209.44.33");
#endif
...
/* Set up the input, output, and exception sets for select(). */
		...
		comm = get_buffer(MAX_STRING_LENGTH * 2);
	    FOREACH(DescriptorList, descriptor_list, iter)
	    {
	    	DescriptorData *d = *iter;
...
//moreinput:
			...
			do
			{
				...
				d->wait -= (d->wait > 0);
				if (d->character) d->character->move_wait -= (d->character->move_wait > 0);
				if (d->wait > 0)							break;//continue;
				if (!d->m_InputQueue.get(comm, aliased))	break;//continue;
				if (d->character) {
				//	Reset the idle timer & pull char back from void if necessary
					d->character->GetPlayer()->m_IdleTime = 0;
					#if 0
					if (/*(STATE(d) == CON_PLAYING)*/ d->GetState()->IsPlaying() && GET_WAS_IN(d->character)) {
						if (d->character->Inside())
							d->character->From(d->character->Inside());
						d->character->To(GET_WAS_IN(d->character));
						d->character->SetWasInRoom(NULL);
				...
				else if (/*STATE(d) != CON_PLAYING*/ !d->GetState()->IsPlaying())	d->GetState()->Parse(comm); //nanny(d, comm);			//	in menus, etc.
				else {														//	We're playing normally
					if (aliased)	d->m_PromptState = DescriptorData::HasPrompt;	//	to prevent recursive aliases
					else if (!d->m_ScriptPrompt.empty())
...
void record_usage(void) {
	...
	log("nusage: %-3ld sockets connected, %-3d sockets playing",
...
void prompt_str(Things::Thing *ch, char *buf, int maxlength) {
	Things::Thing *vict = TARGETING(ch);  
...
//					break;
				case 'T':
				case 't': // tank
					if (vict && (tank = TARGETING(vict)) && tank != ch) {
...
char *make_prompt(DescriptorData *d) {
	...
	if (d->m_Pager)
	{
		sprintf(prompt, "\r`n[ Return to continue, (q)uit, (r)efresh, (b)ack, or page number (%d/%d) ]\r\n",
	...
	{
		*prompt = '\0';
		if (!(d->character))	return prompt;

		if (!d->m_ScriptPrompt.empty())
		{
			strcat(prompt, d->m_ScriptPrompt.c_str());
			if (*(END_OF(prompt) - 1) != ' ')
				strcat(prompt, " ");
		}
		else
		{
			...
			if (!IS_NPC(d->character) && !d->character->GetPlayer()->m_AFKMessage.empty())
				strcat(prompt, "`C[AFK]`n");
			
			if (d->character->GetPlayer()->m_StaffInvisLevel > 0)
				sprintf_cat(prompt, "`gi%d`n ", d->character->GetPlayer()->m_StaffInvisLevel);
			
			static const AffFlags MAKE_BITSET(DisplayedFlags, AFF_INVISIBLE, AFF_INVISIBLE_2, AFF_HIDE, AFF_COVER, AFF_TRAITOR);
			if (AFF_FLAGS(d->character).testForAny(DisplayedFlags)
				/* && !(GET_INVIS_LEV(d->character)) */)
			{
				strcat(prompt, "(");
				if (AFF_FLAGGED(d->character, AFF_INVISIBLE) | AFF_FLAGGED(d->character, AFF_INVISIBLE_2))
					strcat(prompt, "`wi`n");
				if (AFF_FLAGGED(d->character, AFF_HIDE))
					strcat(prompt, "`KH`n");
				if (AFF_FLAGGED(d->character, AFF_COVER))
					strcat(prompt, "`KC`n");
				if (AFF_FLAGGED(d->character, AFF_TRAITOR))
					strcat(prompt, "`RT`n");
				strcat(prompt, ") ");
			}
			
			prompt_str(d->character, prompt, sizeof(prompt) / 4);
		}
...
//	struct hostent *from;
	...
	while (!login.empty() && login[login.length() - 1] == '\n')
	{
		...
		if (!login.empty() && login[login.length() - 1] == '\r')
			login.erase(login.length() - 1);
	}
...
//		SEND_TO_SOCKET("  This may take a moment.", desc);
//	SEND_TO_SOCKET("\r\n", desc);
#endif
...
void check_idle_passwords(void) {
    FOREACH(DescriptorList, descriptor_list, iter)
    {
		...
		if (d->character && PLR_FLAGGED(d->character, PLR_NOSHOUT) && d->character->GetPlayer()->m_MuteLength != 0)
		{
			if ((int)(time(0) - d->character->GetPlayer()->m_MutedAt) >= d->character->GetPlayer()->m_MuteLength)
			{
				REMOVE_BIT(PLR_FLAGS(d->character), PLR_NOSHOUT);
				mudlogf(BRF, LVL_STAFF, TRUE, "(GC) Squelch auto-off for %s.", d->character->GetName());
				d->character->GetPlayer()->m_MuteLength = 0;
			}
		}
...
void send_to_char(const char *messg, Things::Thing *ch) {
	if (!messg || !ch)
		return;
...
void send_to_zone(char *messg, ZoneData *zone) {
	if (!messg || !*messg)
		return;
    ...
    FOREACH(DescriptorList, descriptor_list, iter)
    {
		...
		if (/*(STATE(i) == CON_PLAYING)*/ i->GetState()->IsPlaying() && i->character && AWAKE(i->character) &&
				!PLR_FLAGGED(i->character, PLR_WRITING) &&
				i->character->Inside() &&
				i->character->InZone() == zone)
			i->Write(messg);
	}
}
...
#if 0
// FIXME - we want something like this, right?
void send_to_zone_affiliated(char *messg, ZoneData *zone, IDNum id) {
	if (!messg || !*messg)
		return;
...
void send_to_players(Things::Thing *ch, const char *messg, ...) {
	va_list args;
    ...
    {
    	DescriptorData *i = *iter;
		if (/*(STATE(i) == CON_PLAYING)*/ i->GetState()->IsPlaying() && i->character && i->character != ch &&
				!PLR_FLAGGED(i->character, PLR_WRITING))
			i->Write(send_buf);
	}
}
...
void perform_act(const char *original, Things::Thing *ch, ObjData *obj, const void *vict_obj, Things::Thing *to, int type, char *storeBuf) {
	const char *i = NULL;
	Relation relation = RELATION_NONE;
  	Things::Thing *victim = NULL;
  	Things::Thing *target = NULL;
  	char *arg = NULL;
	char *buf;
	const char *orig = original;
	
	if (IS_SET(type, TO_IGNORABLE) && ch && to->desc && to->desc->IsIgnoring(GET_ID(ch)))
		return;
	...
	while ((c = *orig++))
	{
		...
		else
		{
			...
			{
				IDNum	idnum;
			  	Things::Thing *uid_vict = NULL;
			  	ObjData *uid_obj = NULL;
				
				idnum = atoi(orig);
				
				if (!(uid_vict = Things::Thing::Find(idnum)))
			...
			else
			{
				switch (c)
				{
					case 'n':
						...
						if (type & TO_IGNOREINVIS)		i = (GET_TRUST(to) >= ch->GetPlayer()->m_StaffInvisLevel ? (ch)->GetName() : "someone");
						else							i = PERS(ch, to);
						break;
					case 'N':
						if (type & TO_IGNOREINVIS) {
							CHECK_NULL(vict_obj, (GET_TRUST(to) >= ((Things::Thing *)vict_obj)->GetPlayer()->m_StaffInvisLevel ? ((Things::Thing *)vict_obj)->GetName() : "someone"));
						} else {
							CHECK_NULL(vict_obj, PERS((Things::Thing *) vict_obj, to));
						}
						
						if (vict_obj != NULL)
							relation = to->GetRelation((Things::Thing *)vict_obj);
						victim = (Things::Thing *) vict_obj;
						break;
					case 'm':
						i = HMHR(ch, to);
						break;
					case 'M':
						CHECK_NULL(vict_obj, HMHR((Things::Thing *) vict_obj, to));
						victim = (Things::Thing *) vict_obj;
						break;
					case 's':
						i = HSHR(ch, to);
						break;
					case 'S':
						CHECK_NULL(vict_obj, HSHR((Things::Thing *) vict_obj, to));
						victim = (Things::Thing *) vict_obj;
						break;
					case 'e':
						i = HSSH(ch, to);
						break;
					case 'E':
						CHECK_NULL(vict_obj, HSSH((Things::Thing *) vict_obj, to));
						victim = (Things::Thing *) vict_obj;
						break;
					case 'o':
...
#define SENDOK(ch) ((IS_NPC(ch) || ((ch)->desc && /*(STATE(ch->desc) == CON_PLAYING)*/ ch->desc->GetState()->IsPlaying())) \
		&& (AWAKE(ch) || sleep) && !PLR_FLAGGED((ch), PLR_WRITING))


void act(const char *str, int hide_invisible, Things::Thing *ch, Things::Thing *obj, const void *vict_obj, int type, char *storeBuf)
{
	Things::Thing *			to;
	Things::Thing *			room;
	bool 				sleep = false;
	...
	if (IS_SET(type, TO_VICT)) {
		if ((to = (Things::Thing *) vict_obj) && SENDOK(to) &&
				!(hide_invisible && ch && to->CanSense(ch) < minVisLevel))
	...
	if (IS_SET(type, TO_ZONE | TO_GAME)) {
	    FOREACH(DescriptorList, descriptor_list, iter)
	    {
			...
			if (i->character && SENDOK(i->character) && (i->character != ch) &&
					!(hide_invisible && ch && i->character->CanSense(ch) < minVisLevel) &&
					((IS_SET(type, TO_GAME)) || (ch->InZone() == i->character->InZone())))
				perform_act(str, ch ? ch : i->character, obj, vict_obj, i->character, type, storeBuf);
		}
	}
	...
	if (IS_SET(type, TO_TEAM)) {
	    FOREACH(DescriptorList, descriptor_list, iter)
	    {
	    	DescriptorData *i = *iter;
			if (i->character && SENDOK(i->character) && (i->character != ch) &&
					(GET_TEAM(i->character) == (int)vict_obj))
				perform_act(str, ch ? ch : i->character, obj, vict_obj, i->character, type, storeBuf);
		}
	}
	#endif
	...
	if (IS_SET(type, TO_ROOM | TO_NOTVICT)) {
		...
		FOREACH(ThingList, room->people, iter)
		{
			to = *iter;
			if (SENDOK(to) && !(hide_invisible && ch && to->CanSense(ch) < minVisLevel) &&
	...
	for (i = 0; tokens[i + 1]; i++) {
		...
		switch (type[i]) {
			case '~':
				...
				else						strcat(string, PERS((Things::Thing *) otokens[i], ch));
				break;
			...
			case '@':
				if (!otokens[i])			strcat(string, "someone's");
					...
					strcat(string, PERS((Things::Thing *) otokens[i], ch));
					strcat(string, "'s");
				}
				break;

			case '#':
				if (!otokens[i] || ch->CanSense((Things::Thing *) otokens[i]) == VISIBLE_NONE)
											strcat(string, "its");
				else if (otokens[i] == ch)	strcat(string, "your");
				else						strcat(string, HSHR((Things::Thing *) otokens[i]));
				break;
			...
			case '&':
				...
				else						strcat(string, HSSH((Things::Thing *) otokens[i]));
				break;
			...
			case '*':
				...
				else						strcat(string, HMHR((Things::Thing *) otokens[i]));
				break;
...
void sub_write(char *arg, Things::Thing *ch, bool find_invis, int targets) {
	...
	if (arg) {
			...
			switch (*p) {
				...
				case ':':	//	get obj_data, move to next token
					...
					*s = '\0';
					...
					((obj = get_obj_in_list_vis(ch, name, ch->Inside()->contents)) ?
		...
		*s = '\0';
		...
		if (IS_SET(targets, TO_ROOM)) {
			FOREACH(ThingList, ch->Inside()->people, iter)
			{
				to = *iter;
				if (to != ch && SENDOK(to))
...
#if 0
const char		enable_compress_str[] = { (char)IAC, (char)SB, TELOPT_COMPRESS, (char)WILL, (char)SE, 0 };
const char		enable_compress2_str[] = { (char)IAC, (char)SB, TELOPT_COMPRESS2, (char)IAC, (char)SE, 0 };

void compressStart(DescriptorData *t, int compressionMode)
{
	int derr;

	if (compressionMode && t->compressionMode)
		return;
	
	/* Notify client. */
	if (compressionMode == TELOPT_COMPRESS2)	SEND_TO_SOCKET(enable_compress2_str,t->m_Socket);
	else										SEND_TO_SOCKET(enable_compress_str, t->m_Socket);
	...
	t->deflate->next_in = (unsigned char *)t->small_outbuf;
	t->deflate->next_out = (unsigned char *)t->small_outbuf;
	...
	{
		t->compressionMode = compressionMode;
	}
}


void compressEnd(DescriptorData *t)
{
	int derr;

	if (!t->compressionMode)
		return;
	...
	if (pending)
		write_to_descriptor(t->m_Socket, t->small_outbuf, 6);
...
bool compressToggle( Things::Thing *ch )
{
	if (!ch->desc)
	{
	...
	else if (!ch->desc->compressionMode) {
		SEND_TO_SOCKET( compress2_on_str, ch->desc->m_Socket );
		SEND_TO_SOCKET( compress_on_str, ch->desc->m_Socket );
		return true;
	}
	else
	{
