...
static bool IndexMapBoundFunc(const IndexBase *b, Hash zone)
{
...
//	if (m_Size == 0)	return m_Index;
	...
	return std::lower_bound(m_Index, m_Index + m_Size, zone, IndexMapBoundFunc);
}
...
//	if (m_Size == 0)	return m_Index;
	...
	return std::lower_bound(m_Index, m_Index + m_Size, zone + 1, IndexMapBoundFunc);
}
...
void IndexMapBase::insert(value_type i)
{
	...
	if (!m_bDirty)
	{
		m_bDirty = (m_Size > 0 && i->m_Virtual < m_Index[m_Size - 1]->m_Virtual);	
	}
