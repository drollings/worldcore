...
#define lock_buffers()		(Buffer::ListLock(__PRETTY_FUNCTION__, __LINE__))
#define unlock_buffers()	(Buffer::ListUnlock(__PRETTY_FUNCTION__, __LINE__))
#define LOCK(buf, type)		buf->Lock((type), __PRETTY_FUNCTION__, __LINE__)
#define UNLOCK(buf)			buf->Unlock(__PRETTY_FUNCTION__, __LINE__)
...
static size_t round_to_pow2(size_t y)
{
	...
	if (IS_SET(Buffer::options, BUF_VERBOSE))	log("BUF: round_to_pow2: %ld -> %ld", y, v);
...
//	Lock(Buffer::tBuffer);
	...
	for (int magnitude = 0; magnitude <= buffer_array[BUF_MAX]; magnitude++) {
		for (Buffer *relbuf = Buffer::buf[magnitude]; relbuf; relbuf = relbuf->next) {
			...
			log("BUF: %s:%d forgot to release %ld bytes in %p.", relbuf->who ? relbuf->who : "UNKNOWN", relbuf->line, relbuf->size, relbuf);
			relbuf->Clear();
			UNLOCK(relbuf);
		}
	}
...
Buffer::Type Buffer::Clear(void) {
	...
	if (data[req_size] != MAGIC_NUMBER) {
		...
		log("BUF: SYSERR: ... ticketed for doing %ld in a %ld (%ld) zone.",
		...
		if (IS_SET(Buffer::options, BUF_CHECK)) {
			...
			log("BUF: %s:%d used %d/%ld bytes in %p.",
...
void Buffer::Detach(const char *data, Buffer::Type type, const char *func, const int line_n) {
	...
	buf = Buffer::Find(data, type);
	
	if (!buf)
		log("BUF: SYSERR: Buffer::Detach: No buffer->data %p found for %s:%d.", data, func, line_n);
	else
		buf->Buffer::Detach(func, line_n);
}
...
void Buffer::Detach(const char *func, const int line_n)
{
	...
	if (LOCKED(this)) {
		if (LOCKED(this) == lockWillFree)
			log("BUF: Buffer::Detach: Buffer %p, requested by %s:%d, already slated to be freed.", this, func, line_n);
		else if (LOCKED(this) == lockWillClear)
			log("BUF: Buffer::Detach: Buffer %p, requested by %s:%d, already being detached.", this, func, line_n);
		else if (LOCKED(this) == lockWillRemove)
			log("BUF: Buffer::Detach: Buffer %p, requested by %s:%d, already being removed from list.", this, func, line_n);
		else
			log("BUF: Buffer::Detach: Buffer %p, requested by %s:%d, already locked.", this, func, line_n);
	} else 
#endif
	if (!this->who)
		log("BUF: SYSERR: Buffer::Detach: Buffer %p, requested by %s:%d, already released. Locked: %d", this, func, line_n, LOCKED(clear));
	else {
		LOCK(this, lockWillClear);
		if (IS_SET(Buffer::options, BUF_VERBOSE | BUF_DETAIL))
			log("BUF: %s:%d released %ld bytes in '%s' (%p) from %s:%d.", func, line_n,
					this->size, this->type == Buffer::tMalloc ? this->var : "a buffer",
					this, this->who ? this->who : "UNKNOWN", this->line);
		Clear();
		UNLOCK(this);
	}
} 
...
Buffer::~Buffer(void) {
	if (type == tPersist)
		log("BUF: SYSERR: %p->~Buffer: Freeing %ld byte persistant buffer.", this, size);
	else if (type == tMalloc) {
		if (IS_SET(Buffer::options, BUF_VERBOSE | BUF_DETAIL))
			log("BUF: %p->~Buffer(): Freeing %ld bytes in '%s' from %s:%d.", this, size, var, who, line);
	} else if (IS_SET(Buffer::options, BUF_VERBOSE | BUF_DETAIL))
		log("BUF: %p->~Buffer(): Freeing %ld bytes in expired buffer.", this, size);
...
//	size = round_to_pow2(size);
	...
	if (IS_SET(Buffer::options, BUF_VERBOSE | BUF_DETAIL))
		log("BUF: find_free_buffer: %d scans for %ld bytes, found %p.", scans, size, search);
 ...
 */
char *Buffer::Acquire(size_t size, Buffer::Type type, const char *varname, const char *who, int line)
{
	Buffer *give = Buffer::AcquireBuffer(size, type, varname, who, line);
 ...
 * is not available, one is created.
 */
Buffer *Buffer::AcquireBuffer(size_t size, Buffer::Type type, const char *varname, const char *who, int line)
{
	Buffer *	give;
  ...
  buffer_sanity_check(NULL);
	...
	if (size > BUFFER_MAXSIZE)
		log("BUF: %s:%d requested %ld bytes for '%s'.", who, line, size, varname);
	...
	if (IS_SET(Buffer::options, BUF_VERBOSE | BUF_DETAIL))
		log("BUF: %s:%d requested %ld bytes for '%s', received %ld in %p.", who, line, size, varname ? varname : "a buffer", give->size, give);
 ...
 */
void show_buffers(Things::Thing *ch, Buffer::Type buffer_type, int display_type) {
	Buffer *disp, **head = NULL;
	int i;
	const char *buf_type[] = { "Buffer", "Persist", "Malloc" };
	...
	if (display_type == 1 || display_type == 2) {
		for (int size = MIN_ALLOC; size <= MAX_ALLOC; size++) {
			...
			for (i = 0, bytes = 0, disp = head[magnitude]; disp; disp = disp->next, i++)
				bytes += disp->size;
			log("%5d bytes (%2d): %5d items, %6d bytes, %5ld overhead.", (1 << size), magnitude, i, bytes, sizeof(Buffer) * i);
		}
		return;
	}
...
void Buffer::ListLock(const char *func, int line) {
	if (IS_SET(Buffer::options, BUF_VERBOSE))
...
void Buffer::ListUnlock(const char *func, int line) {
	if (IS_SET(Buffer::options, BUF_VERBOSE))
		log("BUF: List unlocked by %s:%d.", func, line);
	pthread_mutex_unlock(&listmutex);
}

void Buffer::Lock(Buffer::tLock locktype, const char *func, int line) {
	if (!this)
...
void Buffer::Unlock(const char *func, int line) {
	if (!this)
